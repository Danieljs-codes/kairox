...
export const ticketStatusEnum = pgEnum('ticket_status', ['VALID', 'CHECKED_IN', 'REFUNDED']);
...
export const organizer = pgTable(
	...
	(table) => [
		...
		check('organizer_availableBalance_check', sql`${table.availableBalance} >= 0`),
...
export const ticketType = pgTable(
	...
	(table) => [
		...
		check('ticketType_soldCount_quantity_check', sql`${table.soldCount} <= ${table.quantity}`),
	],
);
...
export const order = pgTable(
	'order',
	{
		...
		paystackFee: bigint('paystack_fee', { mode: 'number' }).default(0).notNull(),
...
export const savedEvent = pgTable(
	...
	(table) => [
		...
		uniqueIndex('savedEvent_userId_eventId_idx').on(table.userId, table.eventId),
	],
);
